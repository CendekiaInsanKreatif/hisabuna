...
<aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-secondary-200 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-strong lg:shadow-medium" id="sidebar">
    ...
    <div class="flex flex-col h-full">
        ...
        <div class="flex-shrink-0 px-4 py-6 border-b border-secondary-200">
            <div class="flex items-center justify-center">
                <div class="text-center">
                    <img src="<?php echo e(asset('images/brand/logo-hisabuna-color.svg')); ?>" alt="Hisabuna" class="h-8 w-auto mx-auto mb-2">
        ...
        <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
                            ...
                            <?php $__currentLoopData = $menu->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if($child->is_show == 1): ?>
                                    ...
                                    <a href="<?php echo e($child->route === '#' ? '#' : route($child->route)); ?>"
                                        class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-300 <?php echo e($isActive ? 'bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-emerald' : 'text-secondary-700 hover:text-primary-600 hover:bg-primary-50 hover:shadow-soft'); ?> <?php echo e($isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-[1.02]'); ?>"
                                        <?php echo e($isDisabled ? 'aria-disabled=true' : ''); ?>>

                                        <div class="flex-shrink-0 w-5 h-5 mr-3 flex items-center justify-center">
                                            <img src="<?php echo e(asset($child->icon)); ?>"
                                                 alt="<?php echo e($child->name); ?>"
                                                 class="w-4 h-4 <?php echo e($isActive ? 'filter brightness-0 invert' : 'opacity-70 group-hover:opacity-100 transition-opacity duration-200'); ?>" />
                                        </div>

                                        <span class="flex-1 truncate font-medium"><?php echo e($child->name); ?></span>
            ...
            <?php else: ?>
                <?php $__currentLoopData = $menus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($menu->roles == auth()->user()->roles): ?>
                        <div class="mb-4">
                            <!-- Menu Category Header -->
                            <div class="px-2 py-2 mb-3">
                                <h3 class="text-xs font-bold text-secondary-600 uppercase tracking-wider flex items-center">
        ...
        <div class="px-4 mb-4">
            <div class="border-t border-secondary-200 pt-4">
                ...
                <div class="space-y-1">
                    <?php $isInvoiceActive = $currentRoute === 'invoices.index'; ?>
                    <a href="<?php echo e(route('invoices.index')); ?>"
                       class="group flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-300 hover:scale-[1.02] <?php echo e($isInvoiceActive ? 'bg-gradient-to-r from-info-500 to-info-600 text-white shadow-blue' : 'text-secondary-700 hover:text-info-600 hover:bg-info-50 hover:shadow-soft'); ?>">
                        <div class="flex-shrink-0 w-5 h-5 mr-3 flex items-center justify-center">
                            <svg class="w-4 h-4 <?php echo e($isInvoiceActive ? 'text-white' : 'text-secondary-400 group-hover:text-info-500'); ?> transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<?php /**PATH /var/www/hisabuna/backend/resources/views/layouts/navigation.blade.php ENDPATH**/ ?>