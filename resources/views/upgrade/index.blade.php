...
@section('content')
    <section class="pricing flex bg-superlight gap-5 overflow-x-scroll px-5 md:px-16 snap-x">
        ...
        @foreach ($plans as $plan)
            <div class="tier bg-white p-4 rounded-xl flex flex-col gap-5 min-w-[340px] w-full snap-center justify-between" data-tier="{{ $plan['name'] }}">
                <div>
                    <h3 class="text-2xl text-emerald-800">{{ $plan['name'] }}</h3>
                    <p class="text-emerald-600 font-medium pb-3 border-b border-slate-300 text-xl">{{ $plan['price'] }} <span class="text-sm text-gray-500">/bulan</span></p>
                    <p class="my-2">{{ $loop->first ? 'Fitur dasar:' : 'Semua fitur yang ada pada ' . $plans[$loop->index - 1]['name'] . ', ditambah:' }}</p>
                ...
                <div class="flex gap-3">
                    <a href="{{ $plan['buttonLink'] }}" class="w-full">
