...
@if(session('show_expiration_modal') && $daysUntilExpiration !== null)
    <div id="expirationModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p class="mb-4">
                ...
                <strong>{{ floor($daysUntilExpiration) }} hari</strong>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <h1 class="text-3xl font-bold mb-6 flex items-center gap-3 text-secondary-800">
        ...
        Selamat datang, {{ auth()->user()->name }} ({{ auth()->user()->company_name ?? 'Nama PT Tidak Ada' }})
    </h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-info-500 to-info-600 p-6 rounded-xl shadow-blue text-white hover:shadow-blue-lg transition-all duration-300 transform hover:scale-105">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-sm font-medium bg-white/20 px-2 py-1 rounded-full uppercase tracking-wide">{{ auth()->user()->profile ?? 'Basic' }}</span>
            </div>
            <h3 class="text-sm font-medium text-info-100 mb-1">Status Akun</h3>
            <p class="text-2xl font-bold">{{ $masaAktif }}</p>
            <p class="text-xs text-info-100 mt-2">Sampai: {{ $formattedExpiredDate }}</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-success-500 to-success-600 p-6 rounded-xl shadow-emerald text-white hover:shadow-emerald-lg transition-all duration-300 transform hover:scale-105">
            ...
            <p class="text-2xl font-bold">{{ number_format($totalJurnalComparison['thisYear'] ?? 0) }}</p>
            <p class="text-xs text-success-100 mt-2">Periode {{ auth()->user()->periode }}</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl shadow-purple text-white hover:shadow-purple-lg transition-all duration-300 transform hover:scale-105">
            ...
            <p class="text-2xl font-bold">{{ number_format($dashboardStats['totalCoa'] ?? 0) }}</p>
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 p-6 rounded-xl shadow-orange text-white hover:shadow-orange-lg transition-all duration-300 transform hover:scale-105">
            ...
            <p class="text-2xl font-bold">{{ number_format($dashboardStats['totalTransaksiMonth'] ?? 0) }}</p>
            <p class="text-xs text-orange-100 mt-2">{{ now()->format('F Y') }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-soft hover:shadow-medium transition-all duration-300 border border-secondary-200/50">
            <div class="flex items-center justify-between mb-4">
                ...
                @endphp
                <span class="text-xs font-medium px-2 py-1 rounded-full {{ $saldo >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
                    {{ $saldo >= 0 ? 'Positif' : 'Negatif' }}
                </span>
            </div>
            <p class="text-2xl font-bold {{ $saldoColor }} mb-2">
                Rp {{ number_format(abs($saldo), 0, ',', '.') }}
            </p>
            <p class="text-sm text-secondary-500">Saldo berjalan periode {{ auth()->user()->periode }}</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-soft hover:shadow-medium transition-all duration-300 border border-secondary-200/50">
            ...
            <p class="text-2xl font-bold text-green-600 mb-2">
                Rp {{ number_format($dashboardStats['totalPendapatan'] ?? 0, 0, ',', '.') }}
            </p>
            <p class="text-sm text-secondary-500">Periode {{ auth()->user()->periode }}</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-soft hover:shadow-medium transition-all duration-300 border border-secondary-200/50">
            ...
            <p class="text-2xl font-bold text-red-600 mb-2">
                Rp {{ number_format($dashboardStats['totalPengeluaran'] ?? 0, 0, ',', '.') }}
            </p>
            <p class="text-sm text-secondary-500">Periode {{ auth()->user()->periode }}</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-soft hover:shadow-medium transition-all duration-300 border border-secondary-200/50 mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="{{ route('jurnal.create') }}" class="flex items-center gap-3 p-4 rounded-lg bg-gradient-to-r from-primary-50 to-primary-100 hover:from-primary-100 hover:to-primary-200 text-primary-700 transition-all duration-200 border border-primary-200">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                </svg>
                <span class="font-medium">Buat Jurnal Baru</span>
            </a>
            <a href="{{ route('jurnal.index') }}" class="flex items-center gap-3 p-4 rounded-lg bg-gradient-to-r from-secondary-50 to-secondary-100 hover:from-secondary-100 hover:to-secondary-200 text-secondary-700 transition-all duration-200 border border-secondary-200">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
            ...
            <div class="space-y-3">
                @forelse($topExpenses as $expense)
                    <div class="flex items-center justify-between p-3 rounded-lg bg-secondary-50 hover:bg-secondary-100 transition-colors duration-200">
                        <div class="flex-1">
                            <p class="font-medium text-secondary-800">{{ $expense->category }}</p>
                            <p class="text-sm text-secondary-500">{{ $expense->account_code }}</p>
                        </div>
                        <div class="text-right">
                            <p class="font-bold text-red-600">Rp {{ number_format($expense->total_amount, 0, ',', '.') }}</p>
                        </div>
                    </div>
                @empty
                ...
                @forelse($recentActivities as $activity)
                    <div class="flex items-start gap-3 p-3 rounded-lg bg-secondary-50 hover:bg-secondary-100 transition-colors duration-200">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="font-medium text-secondary-800">{{ $activity['activity'] }}</p>
                            <p class="text-sm text-secondary-500 truncate">{{ $activity['description'] }}</p>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        try {
            ...
            const revenuesData = @json($revenues ?? []);
    ...
    @if(isset($daysUntilExpiration) && $daysUntilExpiration <= 30 && $daysUntilExpiration > 0)
    document.addEventListener('DOMContentLoaded', function() {
