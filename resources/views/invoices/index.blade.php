            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($invoices as $invoice)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm text-gray-700">{{ $invoice->order_id }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700">{{ $invoice->created_at->format('d M Y') }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700">Rp {{ number_format($invoice->amount, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            <div class="space-x-2">
                                {{-- Tombol Download --}}
                                @if(isset($invoice) && file_exists(storage_path('app/' . $invoice->file_path)))
                                ...
                                {{-- Tombol Preview --}}
                                <a href="{{ route('invoice.preview', $invoice->order_id) }}"
                                target="_blank"
