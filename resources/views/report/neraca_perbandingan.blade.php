...
<body>
    @include('report.partials.header', [
        'reportTitle' => 'Neraca Perbandingan',
        'reportPeriod' => 'Tahun ' . $tahunSebelumnya . ' dan ' . $tahunSekarang,
    ])
        ...
        <tbody>
            @foreach ($data[$tahunSekarang] as $keterangan => $subData)
                ...
                @foreach ($subData as $kategori => $nilai)
                    @if (is_array($nilai))
                            ...
                            @if ($subkategori != 'Total')
                                <tr class="indent-2">
                                    ...
                                    <td style="text-align: right;">
                                        {{ number_format($data[$tahunSebelumnya][$keterangan][$kategori][$subkategori], 0, ',', '.') }}
                                    </td>
                                </tr>
                            @endif
                        @endforeach
                        <tr class="total indent-1">
                            ...
                            <td style="text-align: right;">
                                {{ number_format($data[$tahunSebelumnya][$keterangan][$kategori]['Total'], 0, ',', '.') }}
                            </td>
                        </tr>
                    @else
                        {{-- <tr class="indent-1">
                ...
                <tr class="total">
                    ...
                    <td style="text-align: right;">
                        {{ number_format($data[$tahunSebelumnya][$keterangan]['Total'], 0, ',', '.') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
