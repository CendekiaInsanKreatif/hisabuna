...
<head>
    ...
    <title>{{ config('app.name', 'Hisabuna') }}</title>
...
<body class="h-full bg-gradient-to-br from-secondary-50 via-primary-50 to-info-50 font-sans antialiased">
            ...
            <div class="flex items-center justify-between h-16 px-4 lg:px-6">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="lg:hidden flex items-center gap-2 bg-primary-50 rounded-lg px-3 py-1.5 border border-primary-200/50">
                        ...
                        <span class="text-xs font-medium text-primary-800" id="mobile-jakarta-time">{{ now()->setTimezone('Asia/Jakarta')->format('H:i') }}</span>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 lg:gap-4">
                    ...
                    <div class="hidden lg:block text-center bg-gradient-to-r from-primary-50 to-primary-100 rounded-xl px-4 py-2.5 border border-primary-200/50 shadow-soft hover:shadow-medium transition-all duration-300">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-left">
                                <p class="text-sm font-bold text-primary-800 tracking-wider" id="jakarta-time">{{ now()->setTimezone('Asia/Jakarta')->format('H:i:s') }}</p>
                                <p class="text-xs text-primary-600 font-medium">{{ now()->setTimezone('Asia/Jakarta')->format('d M Y') }} • WIB</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <a href="{{ route('profile.edit') }}"
                        ...
                        <div class="relative">
                            <img src="{{ asset('storage/' . Auth::user()->company_logo) }}" alt="User Avatar"
    ...
    <!-- Footer -->
    {{-- <footer class="lg:ml-64 bg-gradient-to-r from-white via-secondary-50 to-white border-t border-secondary-200/60 shadow-soft">
        <div class="px-4 lg:px-6 py-6">
            <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
                <!-- Left Section: Version & Copyright -->
                <div class="flex flex-col lg:flex-row items-center gap-2 lg:gap-4">
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 bg-gradient-to-r from-primary-500 to-primary-600 rounded-lg flex items-center justify-center shadow-soft">
                            <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                        </div>
                        <span class="font-bold text-primary-600 text-sm lg:text-base">HISABUNA</span>
                        <span class="bg-primary-100 text-primary-700 px-2 py-1 rounded-full text-xs font-semibold">v1.0</span>
                    </div>
                    <div class="hidden lg:block w-px h-6 bg-secondary-300"></div>
                    <p class="text-xs lg:text-sm text-secondary-600 text-center lg:text-left">
                        © {{ date('Y') }} Accounting System - All rights reserved
                    </p>
                </div>

                <!-- Right Section: Developer Info -->
                <div class="flex flex-col lg:flex-row items-center gap-2 lg:gap-4">
                    <div class="flex items-center gap-2 text-xs lg:text-sm text-secondary-600">
                        <svg class="w-4 h-4 text-primary-500" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M12.316 3.051a1 1 0 01.633 1.265l-4 12a1 1 0 11-1.898-.632l4-12a1 1 0 011.265-.633zM5.707 6.293a1 1 0 010 1.414L3.414 10l2.293 2.293a1 1 0 11-1.414 1.414l-3-3a1 1 0 010-1.414l3-3a1 1 0 011.414 0zm8.586 0a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 11-1.414-1.414L16.586 10l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd"/>
                        </svg>
                        <span>Developed by</span>
                    </div>
                    <a href="#" class="font-semibold text-primary-500 hover:text-primary-600 transition-colors duration-300 text-sm lg:text-base">
                        PT. Insan Kreatif Cendekia
                    </a>
                </div>
            </div>

            <!-- Additional Info Bar -->
            <div class="mt-4 pt-4 border-t border-secondary-200/60">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-secondary-500">
                    <div class="flex items-center gap-4">
                        <span>Server Time: <span id="server-time">{{ now()->setTimezone('Asia/Jakarta')->format('H:i:s T') }}</span></span>
                        <span class="hidden sm:inline">•</span>
                        <span>Status: <span class="inline-flex items-center gap-1"><span class="w-2 h-2 bg-success-500 rounded-full"></span>Online</span></span>
                    </div>
                    <div class="flex items-center gap-4">
